	...
	const createProposalHistory = async (proposalHistory: ProposalHistory) => {
		try {
			...
			if (!response.ok) {
				...
				throw new Error(errorData?.message ?? "提案履歴の登録に失敗しました。");
			}
	...
	const updateProposalHistory = async (
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(errorData?.message ?? "提案履歴の更新に失敗しました。");
			}
	...
	const deleteProposalHistory = async (proposalHistoryId: number) => {
		try {
			...
			if (!response.ok) {
				...
				throw new Error(errorData?.message ?? "提案履歴の削除に失敗しました。");
			}
	...
	const restoreProposalHistory = async (proposalHistoryId: number) => {
		try {
			...
			if (!response.ok) {
				...
				throw new Error(errorData?.message ?? "提案履歴の復元に失敗しました。");
			}
